...
my $transaction_uuid       		= getProperty($ec, "transaction_uuid", 1);
...
print "Puppet Transaction UUID: '", $transaction_uuid, "'\n";
...
my $url = 'https://' . $puppet_master_url . '/puppet/v3/node/' . $nodename . '?environment=' . $environment .
        '&transaction_uuid=' . $transaction_uuid . '&configured_environment=' . $environment;
